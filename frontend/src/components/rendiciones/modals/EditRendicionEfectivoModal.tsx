...
interface EditModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (item: RendicionEfectivoItem) => void;
  item: RendicionEfectivoItem | null;
}

export const EditRendicionEfectivoModal = ({ isOpen, onClose, onSave, item }: EditModalProps) => {
  const [formData, setFormData] = useState<Partial<RendicionEfectivoItem>>({});
  ...
  const loadDescriptionsForFields = async (item: RendicionEfectivoItem) => {
    const newDescriptions: {[key: string]: string} = {};

    for (const [field, tipoParametro] of Object.entries(modalFieldsWithSelector)) {
      const fieldValue = item[field as keyof RendicionEfectivoItem];
  ...
  // Inicializar formData cuando el item cambia (al abrir el modal)
  useEffect(() => {
    if (item) {
      setFormData({
        fechaComprobante: formatDateForInput(item.fechaComprobante || item.documento?.fechaTransaccion),
        ...
        netoGravado: parseFloat(String(item.netoGravado || 0)) || 0,
        exento: parseFloat(String(item.exento || 0)) || 0,
        importeTotal: parseFloat(String(item.importeTotal || item.documento?.importeExtraido || 0)) || 0,
  ...
  const openSmartSelector = (field: string, element: HTMLElement) => {
    ...
    if (inputElement) {
      ...
      const selectorHeight = 300; // Altura estimada del SmartSelector

      let x = rect.left;
      let y;

      // Si hay espacio suficiente debajo, abrir desde esquina inferior izquierda
      if (spaceBelow >= selectorHeight) {
        y = rect.bottom + window.scrollY;
      }
      // Si no hay espacio debajo pero sí arriba, abrir desde esquina superior izquierda
      else if (spaceAbove >= selectorHeight) {
        y = rect.top + window.scrollY - selectorHeight;
      }
      // Si no hay espacio en ningún lado, abrir debajo (scroll automático)
      else {
        y = rect.bottom + window.scrollY;
      }
      ...
      setSelectorPosition({
        x: rect.left - 200, // Ajustar hacia la izquierda
        y: rect.bottom + window.scrollY
      });
    }
  ...
  const handleSave = () => {
    if (item) {
      const updatedItem: RendicionEfectivoItem = {
        ...item,
        ...formData
      };
      onSave(updatedItem);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg w-full max-w-7xl mx-4 max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <h2 className="text-xl font-semibold text-gray-900">Editar Item de Rendición - Efectivo</h2>
              <div className="flex flex-col gap-1 mt-2">
                <div className="flex items-center gap-4 text-sm text-gray-600">
                  ...
                  <span><strong>Importe:</strong> {new Intl.NumberFormat('es-AR', { style: 'currency', currency: 'ARS' }).format(item.documento?.importeExtraido || item.importeTotal || 0)}</span>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      step="0.01"
                      value={formData.importeTotal ? parseFloat(String(formData.importeTotal)).toFixed(2) : '0.00'}
                      onChange={(e) => { handleInputChange('importeTotal', parseFloat(e.target.value) || 0) }}
                      onBlur={(e) => {
                        const value = parseFloat(e.target.value) || 0;
                        handleInputChange('importeTotal', value);
                      }}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-right"
                    />
                  </div>
        ...
        <div className="px-6 py-4 border-t border-gray-200 bg-gray-50 flex justify-end gap-3">
          ...
          <Button
            onClick={handleSave}
            className="bg-palette-dark hover:bg-opacity-90 text-palette-yellow"
          >
