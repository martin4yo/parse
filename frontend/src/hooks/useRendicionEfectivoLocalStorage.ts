...
interface ChangeTracking {
  ...
  changes: {
    modified: Array<{
      id: string;
      originalData: Partial<RendicionEfectivoItem>;
      currentData: Partial<RendicionEfectivoItem>;
    }>;
    new: Array<{
      tempId: string;
      data: Partial<RendicionEfectivoItem>;
    }>;
    deleted: string[];
  };
}

interface UseRendicionEfectivoLocalStorageProps {
  cajaId: string;
  userId?: string;
  autoSaveDelay?: number;
}

export function useRendicionEfectivoLocalStorage({
  cajaId,
  userId,
  autoSaveDelay = 500
}: UseRendicionEfectivoLocalStorageProps) {
  ...
  const getStorageKey = useCallback(() => {
    ...
    return `rendicion_efectivo_draft_${cajaId}_${userPart}`;
  }, [cajaId, userId]);
  ...
  const trackModification = useCallback((
    ...
    field: string,
    newValue: any,
    originalItem?: RendicionEfectivoItem
  ) => {
    setChanges(prev => {
  ...
  const trackNewItem = useCallback((tempId: string, data: Partial<RendicionEfectivoItem>) => {
    setChanges(prev => ({
      ...prev,
      changes: {
        ...prev.changes,
